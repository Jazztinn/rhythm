...
export function RhythmsView() {
  ...
  return <div className="workspace-view" ref={root}>
    ...
    <section className="rhythm-hero" data-workspace-reveal><div className="rhythm-orbit" aria-hidden="true"><span className="rhythm-orbit-ring"><i /><i /><i /></span><strong>{score}<small>%</small></strong></div><div><span className="section-kicker"><Orbit size={15} /> Today&apos;s flow</span><h2>{activeRhythms.length === 0 ? "Make space for one gentle anchor." : done.length === activeRhythms.length ? "You kept every promise to yourself." : "Consistency, without pressure."}</h2><p>{done.length} of {activeRhythms.length} active rhythms complete. Tomorrow starts fresh automatically.</p><div className="streak"><Flame size={18} /><strong>{done.length ? `${done.length} anchor${done.length === 1 ? "" : "s"} kept today` : "Your first anchor is waiting"}</strong><span>{score}% complete</span></div></div></section>
