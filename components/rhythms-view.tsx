...
import { Archive, Check, Moon, Orbit, Pencil, Plus, Sparkles, Sun, Waves } from "lucide-react";
...
export function RhythmsView() {
  ...
  return <div className="workspace-view" ref={root}>
    ...
    <section className="rhythm-hero" data-workspace-reveal><div className="rhythm-orbit" aria-hidden="true"><span className="rhythm-orbit-ring"><i /><i /><i /></span><strong>{done.length}</strong></div><div><span className="section-kicker"><Orbit size={15} /> Today&apos;s flow</span><h2>{activeRhythms.length === 0 ? "Create something you want to return to." : todayOccurrences.length === 0 ? "Nothing in your Rhythms needs attention today." : done.length === todayOccurrences.length ? "You’re clear for now." : `${todayOccurrences.length - done.length} Rhythm${todayOccurrences.length - done.length === 1 ? "" : "s"} left today.`}</h2><p>Only today&apos;s occurrences appear here. Future occurrences are derived when needed, not stored as tasks.</p></div></section>
