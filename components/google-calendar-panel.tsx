...
export function GoogleCalendarPanel({ start, end, tasks }: { start: string; end: string; tasks: Task[] }) {
  ...
  return <section className="provider-calendar-panel" aria-labelledby="provider-calendar-heading">
    ...
    <div className="provider-event-list">{events.length ? events.map((event) => <article key={`${event.calendarId}:${event.id}`} className={`provider-event ${event.rhythmManaged ? "is-managed" : "is-external"}`}><div><span>{event.rhythmManaged ? "Rhythm managed" : "External · read-only"}</span><strong>{event.summary}</strong><small>{new Intl.DateTimeFormat("en-US", { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(event.start))}</small></div><div className="provider-event__actions">{event.htmlLink ? <a href={event.htmlLink} target="_blank" rel="noreferrer">Open</a> : null}{event.rhythmManaged ? <><Button onClick={() => openEdit(event)}>Edit</Button><ConfirmAction label="Delete" title="Delete this managed event?" description="This deletes only the Rhythm-managed provider event. External events remain read-only and untouched." confirmLabel="Delete event" tone="danger" onConfirm={async () => { const response = await integrationFetch(`/api/integrations/google/events/${encodeURIComponent(event.id)}`, { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ calendarId: event.calendarId }) }); if (response.error) setResult(response as unknown as IntegrationResponse<GoogleEvent[]>); else await refresh(); }} /></> : null}</div></article>) : <p className="provider-empty">No provider events in this visible range. Your local tasks are still shown above.</p>}</div>
