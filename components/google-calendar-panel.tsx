...
import { replaceLocalDate, replaceLocalTime } from "@/lib/barrel-time";
...
export function GoogleCalendarPanel({ start, end, tasks }: { start: string; end: string; tasks: Task[] }) {
  ...
  const unavailable = syncState === "not_configured" || syncState === "disconnected";
  ...
  return <section className="provider-calendar-panel" aria-labelledby="provider-calendar-heading">
    ...
    {result?.error && syncState !== "offline" && !unavailable ? <StatusMessage tone="error">{result.error.message} {result.error.retryable ? <Button onClick={() => void refresh()}>Try again</Button> : null}</StatusMessage> : null}
    ...
    {!unavailable ? <div className="provider-event-list">{events.length ? events.map((event) => <article key={`${event.calendarId}:${event.id}`} className={`provider-event ${event.rhythmManaged ? "is-managed" : "is-external"} ${conflicts[event.id]?.length ? "has-conflict" : ""}`}><div><span>{event.rhythmManaged ? "Scheduled by Rhythm" : "Calendar event · read-only"}</span><strong>{event.summary}</strong><small>{new Intl.DateTimeFormat("en-US", { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(event.start))}</small>{conflicts[event.id]?.map((title) => <p className="provider-conflict" key={title} role="status">Overlaps: {title}</p>)}</div><div className="provider-event__actions">{event.htmlLink ? <a href={event.htmlLink} target="_blank" rel="noreferrer">Open</a> : null}{event.rhythmManaged ? <><Button onClick={() => openEdit(event)}>Edit</Button><ConfirmAction label="Delete" title="Delete this Calendar block?" description="This deletes only the event Rhythm created. You can undo afterward when its task reference is available." confirmLabel="Delete event" tone="danger" onConfirm={() => deleteManagedEvent(event)} /></> : null}</div></article>) : <p className="provider-empty">{result?.status === "connected" ? "No Calendar events in these visible dates." : "Calendar context is not available yet."}</p>}</div> : null}
    {dialog ? <Dialog open onClose={() => setDialog(null)} title={dialog === "preview" ? "Review Calendar change" : editing ? "Edit Calendar block" : "Find a time for this task"} className="calendar-time-dialog">
      {dialog === "preview" ? <>
        <p className="ui-dialog__description">Proposed action: {editing ? "update" : "create"} one Google Calendar event. No other tasks or events will change.</p>
        <p><strong>{summary}</strong><br />{formatSlot({ start: new Date(startInput).toISOString(), end: new Date(endInput).toISOString() })}</p>
        <div className="ui-dialog__actions"><Button onClick={() => setDialog(editing ? "edit" : "create")}>Edit</Button><Button onClick={() => setDialog(null)}>Cancel</Button><Button variant="primary" disabled={saving} onClick={() => void saveManagedEvent()}>{saving ? "Applying…" : "Approve and apply"}</Button></div>
      </> : <>
        <p className="ui-dialog__description">{slots.length && !editing ? "Suggested times use only connected Calendar events in the visible dates. Choose one or enter a time." : "Choose a time, then review before anything changes."}</p>
        {!editing && slots.length ? <div className="provider-selection" role="group" aria-label="Suggested and alternate times">{slots.map((slot) => <button type="button" key={slot.start} onClick={() => applySlot(slot)}>{slot.label} · {formatSlot(slot)}</button>)}</div> : null}
        <label className="integration-field">Task<select value={taskId} onChange={(event) => { setTaskId(event.target.value); const task = tasks.find((item) => item.id === event.target.value); if (task) setSummary(task.title); }}>{tasks.map((task) => <option key={task.id} value={task.id}>{task.title}</option>)}</select></label>
        <label className="integration-field">Title<input value={summary} onChange={(event) => setSummary(event.target.value)} /></label>
        <div className="calendar-time-fields">
          <section><label className="integration-field">Start date<input type="date" value={startInput.slice(0, 10)} onChange={(event) => setStartInput(replaceLocalDate(startInput, event.target.value))} required /></label><BarrelTimePicker value={startInput.slice(11, 16)} onChange={(time) => setStartInput(replaceLocalTime(startInput, time))} label="Calendar start time" /></section>
          <section><label className="integration-field">End date<input type="date" value={endInput.slice(0, 10)} onChange={(event) => setEndInput(replaceLocalDate(endInput, event.target.value))} required /></label><BarrelTimePicker value={endInput.slice(11, 16)} onChange={(time) => setEndInput(replaceLocalTime(endInput, time))} label="Calendar end time" /></section>
        </div>
        <div className="ui-dialog__actions"><Button onClick={() => setDialog(null)}>Cancel</Button><Button variant="primary" disabled={!validDraft} onClick={() => setDialog("preview")}>Review change</Button></div>
      </>}
    </Dialog> : null}
  </section>;
}
