...
import { ArrowUpRight, Check, Circle, Clock3, Pencil, Plus, Search, Sparkles } from "lucide-react";
...
function TaskRow({ task, onToggle, onEdit, exiting = false }: { task: Task; onToggle: () => void; onEdit: () => void; exiting?: boolean }) {
  return <ViewTransition name={`task-${task.id}`} default="none" share="task"><li className={`full-task-row ${task.status === "completed" ? "is-done" : ""} ${exiting ? "is-exiting" : ""}`}>
